...
public class DemultiplexMUSICValidate {
	...
	public static void main(String[] args) {
		...
		File R2 = null;
		//hardcoded
		File barcodeFile = new File("validation_sgRNAs.txt");;
		if(args.length==2) {
		...
		try {
			...
			while(iterR1.hasNext()) {
				...
				if(start>=0 && end>=0 && end > (start+startOfBarCodeString.length())) {
					...
					//remove the last matched nucleotide
					String barcode = R2S.substring(start, end-1);
					String barcodeAlt = R2S.substring(start, end);
					
					//overwrite the barcode if the additional nucleotide is required for the match
					if(barcodes.containsKey(barcodeAlt)) {
						barcode = barcodeAlt;
					}
			...
			//debug print the nonMatched ones
	...
	private static HashMap<String, String> createHashMap(File f) {
		...
		try {
			...
			while(s.hasNextLine()) {
				...
				//System.out.println("Adding ["+revCom+"] "+revCom.length()+"\t"+id);
				hm.put(revCom, id);
			}
			s.close();
		} catch (IOException e) {
